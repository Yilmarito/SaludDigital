...
function selectAnswer(idx) {
    ...
    // Mark each button: correct answer always gets green, chosen wrong answer gets red
    btns.forEach((b, i) => {
    ...
    const wrongPrefix = currentLang === 'es'
        ? 'Incorrecto. '
        : 'Incorrect. ';
    const rightPrefix = currentLang === 'es'
        ? 'Correcto. '
        : 'Correct. ';

    // Strip any existing prefix from q.feedback to avoid duplication
    let feedbackBody = q.feedback
        .replace(/^(Correcto|Exacto|Muy bien|Correct|Exactly|Well done)[.!]?\s*/i, '');

    fb.textContent = isRight
        ? rightPrefix + feedbackBody
        : wrongPrefix + feedbackBody;

    fb.className = 'quiz-feedback show ' + (isRight ? 'ok' : 'fail');
...
(function () {
    ...
    hamburger.addEventListener('click', (e) => {
        ...
        // When closing nav, also close any open mobile dropdown
        if (!navLinks.classList.contains('open')) {
            navLinks.querySelectorAll('.nav-dropdown-menu').forEach(m => m.classList.remove('mob-open'));
        }
    });

    // On mobile, the "Módulos" <a> inside the dropdown should toggle the submenu
    // instead of navigating, since there's no hover on touch screens.
    ...
    navLinks.querySelectorAll('.nav-dropdown > a').forEach(trigger => {
        trigger.addEventListener('click', (e) => {
            ...
            navLinks.querySelectorAll('.nav-dropdown-menu.mob-open').forEach(m => m.classList.remove('mob-open'));
    ...
    // On resize to desktop, clean up mobile classes
